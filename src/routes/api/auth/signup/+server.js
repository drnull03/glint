...
export const POST = async ({ request, fetch }) => {
    const { name, email, password: unhashedPassword } = await request.json();
    const password = await hashPassword(unhashedPassword);
    const userID = await insertUser(name, email, password);
    if(!userID) {
        return json({ status: false, message: "Account creation failed, please try again later" })
    }
